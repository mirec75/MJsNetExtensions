...
namespace MJsNetExtensions.Xml.Serialization
{
    ...
    public class XmlSerializationSettings
    {
        ...
        /// </summary>
        public XmlSerializerNamespaces Namespaces { get; set; } = null;
        ...
        public void UpdateSettingsFrom(string defaultNamespace, bool addSchemaLocationToResultXml)
        {
            ...
            {
                XmlQualifiedName defaultNamespaceQN = this.Namespaces.ToArray()?.FirstOrDefault(it => string.IsNullOrWhiteSpace(it.Name)) ?? null;
                
                if (string.IsNullOrWhiteSpace(defaultNamespaceQN?.Namespace))   //NOTE: !! the "this.Namespaces.Add(...)" also overwrites the setting without an error! Nice!! :)
                {
                ...
                {
                    XmlQualifiedName xsiNamespace = this.Namespaces.ToArray()?.FirstOrDefault(it => string.CompareOrdinal(it.Namespace, XmlSchema.InstanceNamespace) == 0) ?? null;

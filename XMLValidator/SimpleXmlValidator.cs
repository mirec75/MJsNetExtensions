namespace XmlValidatorExe
{
    ...
    public class SimpleXmlValidator
    {
        ...
        private static void PrintProgramHeader()
        {
            ...
            var attributtes = assembly.GetCustomAttributes(false);
            string title = attributtes.OfType<AssemblyTitleAttribute>().Single().Title;
            string description = attributtes.OfType<AssemblyDescriptionAttribute>().Single().Description;
            string copyright = attributtes.OfType<AssemblyCopyrightAttribute>().Single().Copyright;
        ...
        private void PrnUsage()
        {
            ...
            SimpleTracer.Prnln("                  in attribute xsi:schemaLocation. Seperate multiple files with , or ;");
        ...
        private int ValidationTest(string[] args)
        {
            ...
            try
            {
                ...
                if (args.Length > 2)
                {
                    if (args[2].ToUpperInvariant() == "-doNotResolveWebUris".ToUpperInvariant() || args[2].ToUpperInvariant() == "-ignoreWeb".ToUpperInvariant())
                    {
                ...
                if (args.Length > 3)
                {
                    if (string.IsNullOrEmpty(args[3])
                      || (args[3].ToUpperInvariant() != "-doNotResolveWebUris".ToUpperInvariant() && args[3].ToUpperInvariant() != "-ignoreWeb".ToUpperInvariant()))
                    {
                ...
                else if (args[0].Trim().ToLower(ic) == "-dtd")
                {
        ...
        protected void HandleRootElement(object sender, RootElementEventArgs e)
        {
            // ignore for now
        }
        ...
        protected void HandleXmlValidationIssue(object sender, XmlValidationIssueEventArgs e)
        {

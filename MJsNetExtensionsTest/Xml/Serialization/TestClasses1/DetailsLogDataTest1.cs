namespace MJsNetExtensionsTest.Xml.Serialization.TestClasses1
{
    ...
    {
        #region Fields
        public CommonLogDataTest1 ownerFieldPublic = null;
        protected CommonLogDataTest1 ownerFieldProtected = null;
        private CommonLogDataTest1 ownerFieldPrivate = null;
        internal CommonLogDataTest1 ownerFieldInternal = null;
        ...
        [XmlIgnore]
        public CommonLogDataTest1 Owner { get; set; } = null;

        [XmlIgnore]
        public DetailsLogDataTest1[] Siblings { get; set; } = null;

        [XmlIgnore]
        public Dictionary<string, DetailsLogDataTest1> SiblingsDict { get; set; } = null;

        [XmlIgnore]
        public Dictionary<DetailsLogDataTest1, string> ReverseSiblingsDict { get; set; } = null;
        ...
        public bool Equals(DetailsLogDataTest1 that)
        {
            ...
            if (string.Compare(this.DbTableName, that.DbTableName, StringComparison.OrdinalIgnoreCase) != 0)
            {
                return false;
            }
            ...
            if (string.Compare(this.Component, that.Component, StringComparison.OrdinalIgnoreCase) != 0)
            {
                return false;
            }

            if (string.Compare(this.Message, that.Message, StringComparison.OrdinalIgnoreCase) != 0)
            {
                return false;
            }
        ...
        {
            int hash = (this.DbTableName ?? "").GetHashCode();
            hash ^= this.Id.GetHashCode();
            hash ^= this.LogId.GetHashCode();
            hash ^= this.DetailDateTime.GetHashCode();
            hash ^= this.Level.GetHashCode();
            hash ^= (this.Component ?? "").GetHashCode();
            hash ^= (this.Message ?? "").GetHashCode();
